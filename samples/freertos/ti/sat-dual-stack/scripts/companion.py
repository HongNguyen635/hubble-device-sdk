...
Scans for a Hubble device connects, and writes Unix epoch (ms) and orbital parameters data.

Usage:
    python scripts/companion.py
    python scripts/companion.py --label <device_label>
    python scripts/companion.py --label <device_label> -v
...
async def provision(label: str | None) -> bool:
    """
    Provision a Hubble device by scanning, connecting, and writing orbital params and UTC time.
...
def main():
    ...
    success = asyncio.run(provision(args.label))
