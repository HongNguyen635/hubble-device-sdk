...
### 2. Embed Device Key and Location

The device's Hubble key and physical location must be baked into the firmware
at build time. Use the *embed_key_location.py* script to generate
`src/key.c` and `src/location.c`:

```bash
python scripts/embed_key_location.py --key <base64-encoded-key> -o src/
```

The script auto-detects the host's geographic location via IP geolocation.
Edit the `src/location.c` directly with the precise coordinates if desired.
...
and waits for provisioning data. Use *companion.py* to push the current UTC time and the
ephemeris data for the target satellites:
...
```text
       ...
       +-------------------------------+           v
                   ...
                   yes |       |                                    |
                       |       | companion.py writes UTC time +     |
                       |       | satellite orbital params over GATT |
                       |       +------------------------------------+
                       |                           |
                       |<--------------------------+
                       v
